...
plot(mean_gs ~ stomlengthab, data=gmes2, ylim=c(0,.5), xlim=c(20, 50), type='n', ylab="")
points(mean_gs ~ stomlengthab, data=gmes2, col=co2grow, pch=pchs[canopy], cex=1.25)
...
plot(mean_gs ~ spi.ab, data=gmes2, type='n', ylab="", ylim=c(0,.5), xlim=c(2, 8))
points(mean_gs ~ spi.ab, data=gmes2, col=co2grow, pch=pchs[canopy], cex=1.25)
# ablineclip(fit_length_ac2, x1=min(ac$stomlengthab), x2=max(ac$stomlengthab), lty=2, lwd=2)
