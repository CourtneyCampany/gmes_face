...
plot(Photo ~ gmes, data=growCO2, col=co2grow, pch=pchs[canopy], ylim=c(0,30), 
  ...
  ablineclip(fit_agm, lwd=2, lty=2, x1= min(growCO2$mean_gm), x2=max(growCO2$mean_gm), 
...
fit_gmgs <- lm(gmes ~ mean_gs, data = growCO2) 
  summary(fit_gmgs) # non-significant relationship (0.85)  
...
plot(mean_ci~mean_gs, data=growCO2, col=co2grow, pch=pchs[canopy], ylab="",
  ...
  # again, need two lines here for CO2 treatment?
...
ablineclip(fit_gmlength, lwd=2, lty=2, x1= min(growCO2$length.par1.mean), x2=max(growCO2$length.par1.mean)) 
...
fit_gmesleafw <- lm(gmes~leafw.mean.y, data = growCO2) 
summary(fit_gmesleafw) # not-significant, too bad, 
...
#kinda the same relationship as above, just a different axis.
...
gmes3 <-read.csv("Tree-means_Gm-master2.csv") # reload after adjusting names!!
...
#Openness
  ...
  growCO2$log_open <- log10(growCO2$Openness)
mod_open<- lmer(log_open ~ co2grow*position + (1|ring/tree), data=growCO2, na.action = na.omit)
...
mod_mesothick <- lmer(meso.mean.y ~ co2grow*position + (1|ring/tree), data=growCO2, na.action = na.omit)
  ...
  # both have strong position effects (others are ns) ==> why not significant anymore?

mod_mesothick2 <- lmer(mean.mesoThick ~ co2grow*position + (1|ring/tree), data=growCO2, na.action = na.omit)
  Anova(mod_mesothick2, test = "F")
...
mod_stldunder <- lmer(stomlengthab ~ co2grow*position + (1|ring/tree), data=growCO2, na.action = na.omit) #underside stomata
  Anova(mod_stldunder, test = "F") # stomatal length

# stomatal index
mod_spiab <- lmer(spi.ab ~ co2grow*position + (1|ring/tree), data=growCO2, na.action = na.omit) #underside stomata
Anova(mod_spiab, test = "F") # stomatal index
